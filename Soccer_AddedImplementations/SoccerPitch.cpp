                                         ...
                                         m_bGameOn(true)
{
  ...
  //create the goals
   m_pRedGoal  = new Goal(Vector2D( m_pPlayingArea->Left(), (cy-Prm.GoalWidth)/4),
                          Vector2D(m_pPlayingArea->Left(), cy - (cy-Prm.GoalWidth)/4),
                          Vector2D(2,0));
  ...
  //adding in Referee
//m_pGreenTeam = new SoccerTeam(m_pBlueGoal, m_pBlueGoal, this, SoccerTeam::green);
...
bool SoccerPitch::Render()
{
  ...
  gdi->HollowBrush();
  gdi->RedPen();
  gdi->Rect(m_pPlayingArea->Left(), (m_cyClient-Prm.GoalWidth)/4, m_pPlayingArea->Left()+40, m_cyClient - (m_cyClient-Prm.GoalWidth)/4);

  gdi->BluePen();
  gdi->Rect(m_pPlayingArea->Right(), (m_cyClient-Prm.GoalWidth)/1.6, m_pPlayingArea->Right()-40, m_cyClient - (m_cyClient-Prm.GoalWidth)/1.6);
